...
describe("ActivePlanCard", () => {
  it("renders the plan name with the active subscription badge", async () => {
    const wrapper = await render(<ActivePlanCard tier="standard" planName="Ailiv Standard" />);

    expect(wrapper.getByTestId("active-plan-card")).toHaveAttribute("data-tier", "standard");
    expect(wrapper.container.textContent).toContain("Ailiv Standard");
  ...
  it("applies the plus accent class for the plus tier", async () => {
    const wrapper = await render(<ActivePlanCard tier="plus" planName="Ailiv Plus" />);
    const card = wrapper.getByTestId("active-plan-card");
    expect(card.className).toMatch(/activePlanCard--plus/);
  });

  it("applies the pro accent class for the pro tier", async () => {
    const wrapper = await render(<ActivePlanCard tier="pro" planName="Ailiv Pro" />);
    const card = wrapper.getByTestId("active-plan-card");
  ...
  it("renders an error message when isError is true", async () => {
    const wrapper = await render(<ActivePlanCard tier="standard" isError planName="Ailiv Standard" />);
    expect(wrapper.container.textContent).not.toContain("Ailiv Standard");
  });

  it("shows a cancellation badge when cancellationDate is provided", async () => {
    const wrapper = await render(
      <ActivePlanCard tier="plus" planName="Ailiv Plus" cancellationDate="2030-01-15T00:00:00Z" />
    );

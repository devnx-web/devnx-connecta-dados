...
describe("StatusBanner", () => {
  ...
  it("should not render locked banner", async () => {
    ...
    expect(wrapper.container.textContent).toContain("Ailiv Support");
  });
  ...
  it("should render post-trial banner w/o link", async () => {
    ...
    const wrapper = await render(<StatusBanner />);
    expect(wrapper.container.textContent).toContain("Subscribe to Ailiv");
    expect(wrapper.queryByRole("link")).not.toBeInTheDocument();
  });
  ...
  describe("entitlements trial plans behavior", () => {
    ...
    it("should render subscribe text when not using entitlements trial plan", async () => {
      ...
      const wrapper = await render(<StatusBanner />);
      expect(wrapper.container.textContent).toBe(
        "Your trial ends in 5 days. Subscribe today to keep using Ailiv after your trial ends."
      );
      expect(wrapper.queryByRole("link")).toBeInTheDocument();
    });

...
import { useGetDefaultUserAsync, useGetInstanceConfiguration, useSimpleAuthLogin, useSimpleAuthLogout } from "core/api";
...
const loggedOutAuthState: LoggedOutState = {
  user: null,
  inited: true,
  loggedOut: true,
};
...
export const SimpleAuthService: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  const [authState, dispatch] = useReducer(
    simpleAuthStateReducer,
    instanceConfig.version === "dev" ? loggedOutAuthState : initialAuthState
  );
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === "development" && instanceConfig.version === "dev") {
  ...
  }, [getDefaultUser, instanceConfig.version]);
